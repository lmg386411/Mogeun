...
fun FatigueLineGraph(
    ...
    Column(
        ...
        Canvas(
            ...
            val verticalStep = if (absMinY.toFloat() > 20) (absMaxY.toInt() - absMinY.toInt() + 40) / maxPointsSize.toFloat()
            else (absMaxY.toInt() + 20) / maxPointsSize.toFloat()
            ...
            for (i in 0..maxPointsSize) {
                val intervalValue = if (absMinY.toFloat() > 20) (absMinY.toFloat() - 20f + (verticalStep*i)).roundToInt()
...
//                }
            ...
            for (i in 0 until maxPointsSize) {
                ...
                val y1 = if (absMinY.toFloat() > 20) gridHeight - (yItemSpacing * ((yAxisData[i].toFloat() - absMinY.toFloat() + 20f) / verticalStep.toFloat()))
...
@Composable
fun FatigueLineGraph2(
    xAxisData: List<GraphData>,
    yAxisData1: List<Number>,
    yAxisData2: List<Number>,
    header: @Composable() () -> Unit = {},
    style: LineGraphStyle = LineGraphStyle(),
    style2: LineGraphStyle = LineGraphStyle(),
    onPointClicked: (pair: Pair<Any,Any>) -> Unit = {},
) {
    ...
    Column(
        ...
        Canvas(
                ...
                .pointerInput(true) {
                    ...
                    detectTapGestures { p1: Offset ->
                        ...
                        shortest?.let {
                            ...
                            //
                            val index = offsetList.indexOf(it)
                            onPointClicked(Pair(xAxisData[index].text, yAxisData1[index]))
                        }
            ...
            // the maximum points for x and y axis to plot (maintain uniformity)
            val maxPointsSize: Int = minOf(xAxisData.size, yAxisData1.size)

            // maximum of the y data list
            val absMaxY = GraphHelper.getAbsoluteMax(yAxisData1)
            val absMinY = GraphHelper.getAbsoluteMin(yAxisData1)
            println("yAxisData1 - $yAxisData1")
            println("yAxisData2 - $yAxisData2")
...
//                }
            ...
            for (i in 0 until maxPointsSize) {
                ...
                //val x1 = xItemSpacing * pos
                val x1 = if (style.yAxisLabelPosition == LabelPosition.RIGHT) (xItemSpacing * i) else (xItemSpacing * i) + yAxisPadding.toPx()
                val y1 = if (absMinY.toFloat() > 20) gridHeight - (yItemSpacing * ((yAxisData1[i].toFloat() - absMinY.toFloat() + 20f) / verticalStep.toFloat()))
                else gridHeight - (yItemSpacing * (yAxisData1[i].toFloat() / verticalStep.toFloat()))
            ...
            val path1 = Path().apply {
                ...
                // ending point for gradient
                lineTo(
                    x = if (style.yAxisLabelPosition == LabelPosition.RIGHT) xItemSpacing * (yAxisData1.size - 1) else gridWidth,
                    y = gridHeight
                )
            ...
            for (i in 0 until maxPointsSize) {
                ...
                //val x1 = xItemSpacing * pos
                val x1 = if (style.yAxisLabelPosition == LabelPosition.RIGHT) (xItemSpacing * i) else (xItemSpacing * i) + yAxisPadding.toPx()
                val y1 = if (absMinY.toFloat() > 20) gridHeight - (yItemSpacing * ((yAxisData2[i].toFloat() - absMinY.toFloat() + 20f) / verticalStep.toFloat()))
                else gridHeight - (yItemSpacing * (yAxisData2[i].toFloat() / verticalStep.toFloat()))
            ...
            val path = Path().apply {
                ...
                // ending point for gradient
                lineTo(
                    x = if (style.yAxisLabelPosition == LabelPosition.RIGHT) xItemSpacing * (yAxisData2.size - 1) else gridWidth,
                    y = gridHeight
                )
...
@Composable
fun BodyLineGraph(
    xAxisData1: List<GraphData>,
    yAxisData1: List<Number>,
    xAxisData2: List<GraphData>,
    yAxisData2: List<Number>,
    header: @Composable() () -> Unit = {},
    style: LineGraphStyle = LineGraphStyle(),
    style2: LineGraphStyle = LineGraphStyle(),
    ...
    val xAxisData = xAxisData1 + xAxisData2
    val yAxisData = yAxisData1 + yAxisData2
    Log.d("xAxisData", xAxisData.toString())
    ...
    Column(
        ...
        Canvas(
            ...
            // the maximum points for x and y axis to plot (maintain uniformity)
            val maxPointsSize1 = yAxisData1.size
            val maxPointsSize2 = yAxisData2.size
            ...
            val xItemSpacing1 = if (style.yAxisLabelPosition == LabelPosition.RIGHT){
                gridWidth / (maxPointsSize1 - 1)
            } else (gridWidth - yAxisPadding.toPx()) / (maxPointsSize1 - 1)
            val xItemSpacing2 = if (style.yAxisLabelPosition == LabelPosition.RIGHT){
                gridWidth / (maxPointsSize2 - 1)
            } else (gridWidth - yAxisPadding.toPx()) / (maxPointsSize2 - 1)
            val yItemSpacing = gridHeight / (yAxisLabelList.size - 1)
...
//                for (i in 0 until maxPointsSize1) {
//
//                    // lines inclined towards x axis
//
//                    val labelXOffset = if (style.yAxisLabelPosition == LabelPosition.RIGHT) xItemSpacing1 * (i) else (xItemSpacing1 * (i)) + yAxisPadding.toPx()
//
//                    drawLine(
//                        color = Color.LightGray,
//                        start = Offset(labelXOffset, 0f),
...
//                }
            ...
            for (i in 0 until maxPointsSize1) {
                ...
                //val x1 = xItemSpacing * pos
                val x1 = if (style.yAxisLabelPosition == LabelPosition.RIGHT) (xItemSpacing1 * i) else (xItemSpacing1 * i) + yAxisPadding.toPx()
                val y1 = gridHeight - (yItemSpacing * (yAxisData1[i].toFloat() / verticalStep.toFloat()))
            ...
            val path1 = Path().apply {
                ...
                // ending point for gradient
                lineTo(
                    x = if (style.yAxisLabelPosition == LabelPosition.RIGHT) xItemSpacing1 * (yAxisData1.size - 1) else gridWidth,
                    y = gridHeight
                )
            ...
            for (i in 0 until maxPointsSize2) {

                //val pos = if (style.yAxisLabelPosition == LabelPosition.RIGHT) i else (i+1)

                //val x1 = xItemSpacing * pos
                val x1 = if (style.yAxisLabelPosition == LabelPosition.RIGHT) (xItemSpacing2 * i) else (xItemSpacing2 * i) + yAxisPadding.toPx()
                val y1 = gridHeight - (yItemSpacing * (yAxisData2[i].toFloat() / verticalStep.toFloat()))
                ...
                drawCircle(
                    color = style2.colors.pointColor,
                    radius = 5.dp.toPx(),
                    center = Offset(x1, y1)
                )
            }
            ...
            val path2 = Path().apply {
                ...
                // ending point for gradient
                lineTo(
                    x = if (style.yAxisLabelPosition == LabelPosition.RIGHT) xItemSpacing2 * (yAxisData2.size - 1) else gridWidth,
                    y = gridHeight
                )
            ...
            drawPoints(
                points = offsetList.subList(
                    fromIndex = maxPointsSize1,
                    toIndex = maxPointsSize1 + maxPointsSize2
                ),
                color = style2.colors.lineColor,
                pointMode = PointMode.Polygon,
                strokeWidth = 3.dp.toPx(),
            ...
            clickedPoint.value?.let {
                drawCircle(
                    color = style2.colors.clickHighlightColor,
                    center = it,
                    radius = 12.dp.toPx()
                )
